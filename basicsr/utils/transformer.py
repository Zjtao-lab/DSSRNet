...
from basicsr.utils.tools import extract_image_patches,\
...
## Base block
class MLABlock(nn.Module):  # ET   # (b, *, dim)<-(b, n_feat, h, w)
    def __init__(
    ...
    def forward(self, x):
        ...
        x = x.permute(0,2,1)
        # x = x.resize(16, 1728, 768)

        x = x + self.atten(self.norm1(x))  # (16, 1728, 768)
        # x + (Norm -> EMHA)
