...
def paired_random_crop_hw(img_gts, img_lqs, gt_patch_size_h, gt_patch_size_w, scale, gt_path):
    ...
    # randomly choose top and left coordinates for lq patch
    top = random.randint(0, h_lq - lq_patch_size_h)  # 通过随机选择 LQ 图像的左上角坐标 top 和 left，进行 LQ 图像的裁剪。
...
def withhat_paired_random_crop_hw(img_gts, img_lqs, img_reallqs, gt_patch_size_h, gt_patch_size_w, scale, gt_path):
    """Paired random crop.
    ...
    if not isinstance(img_lqs, list):
        img_lqs = [img_lqs]
    if not isinstance(img_reallqs, list):
        img_reallqs = [img_reallqs]
    ...
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    if len(img_reallqs) == 1:
        img_reallqs = img_reallqs[0]
    return img_gts, img_lqs, img_reallqs
...
def img_half_mixup(img_gts, img_lqs):
    """img_half_mixup.
