...
class NAFNetSR(nn.Module):
    ...
    def __init__(self, up_scale=4, width=48, num_blks=16, img_channel=3, drop_path_rate=0., drop_out_rate=0., fusion_from=-1, fusion_to=-1, dual=False):
        ...
        self.fakeup = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=img_channel * up_scale**2, kernel_size=3, padding=1, stride=1, groups=1, bias=True),
            nn.PixelShuffle(up_scale)
        )
    ...
    def forward(self, inp):
        ...
        w_pixelshuffle = True
        if w_pixelshuffle:
            feats = [self.newintro(self.pixelunshuffle(x)) for x in inp]
...
class NAFSSR(Local_Base, NAFNetSR):  # ori
