...
class NAF_SimpleFFCNetSR(nn.Module):
    '''
    NAFNet add FFCBlockSR for Super-Resolution
...
class NewsimpleFFCSSR(Local_Base, NAF_SimpleFFCNetSR):  # NAFNet中的基础模块，simplge gate换成简单的ffb，只有一层ffc
    '''
    NAFNet add FFCBlockSR for Super-Resolution
    NAFNet中的基础模块，simplge gate换成简单的ffb，只有一层ffc
    '''
    def __init__(self, *args, train_size=(1, 6, 30, 90), fast_imp=False, fusion_from=-1, fusion_to=1000, **kwargs):
        Local_Base.__init__(self)
        NAF_SimpleFFCNetSR.__init__(self, *args, img_channel=3, fusion_from=fusion_from, fusion_to=fusion_to, dual=True, **kwargs)
...
if __name__ == '__main__':
    ...
    width = 64
    droppath= 0.1
    train_size = (1, 6, 60, 180) 

    # net = NewSSR(up_scale=2, train_size=train_size, fast_imp=True, width=width, num_blks=num_blks, drop_path_rate=droppath)
    net = NewsimpleFFCSSR(up_scale=2, train_size=train_size, fast_imp=True, width=width, num_blks=num_blks, drop_path_rate=droppath)
    ...
    img = torch.rand(1, 6, 60, 180)
    feat = torch.rand(1, 2, 60, 180)
    output = net(img)
