...
# run， train
# nohup CUDA_VISIBLE_DEVICES=1 python -m torch.distributed.launch \
#     --nproc_per_node=1 --master_port=4321 basicsr/train.py \
...
nohup python -m torch.distributed.launch \
    ...
    -opt options/train_oriHat/NAFSSR-S_x2.yml --launcher pytorch \
    > outprint_simpleffb_oriHat_v0.2.txt 2>&1 &


# test
# python -m torch.distributed.launch --nproc_per_node=1 --master_port=4321 basicsr/test.py -opt options/test/NAFSSR/NAFSSR-S_2x.yml --launcher pytorch
# CUDA_VISIBLE_DEVICES=1 python -m torch.distributed.launch --nproc_per_node=1 --master_port=4321 basicsr/test.py -opt options/test/NAFSSR/NAFSSR-S_2x.yml --launcher pytorch
